...
const registerValidator = Joi.object({
  username: Joi.string()
    .pattern(/^[a-z]+[a-z0-9\_\.]{4}/)
    .required()
    .error(new Error("Enter a valid username.")),
  password: Joi.string()
    .required()
    .min(6)
    .error(new Error("Enter atleast 6 character for password.")),
  confirm_password: Joi.string()
    ...
    email: Joi.string()
    .email()
    .error(new Error("Enter a valid email address."))
    .allow(""),
    mobile: Joi.string()
    .pattern(/^09[0-9]{9}$/)
    .error(new Error("Enter a valid phone number."))
    .allow(""),
